"""Cross-correlograms from Spyking-Circus"""
...
import numpy as np
import phy_functions


def _increment(arr, indices):
    """Increment some indices in a 1D vector of non-negative integers.
    Repeated indices are taken into account."""
    arr = phy_functions._as_array(arr)
    indices = phy_functions._as_array(indices)
...
def correlograms(spike_times,
    ...
    spike_clusters = phy_functions._as_array(spike_clusters)
    ...
    if cluster_ids is None:
        clusters = phy_functions._unique(spike_clusters)
    else:
        clusters = phy_functions._as_array(cluster_ids)
    n_clusters = len(clusters)

    # Like spike_clusters, but with 0..n_clusters-1 indices.
    spike_clusters_i = phy_functions._index_of(spike_clusters, clusters)
